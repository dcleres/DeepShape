function [center_img,sphere_img,sphcen,sphrad]=SphericalHough(img,radrange,varargin)
...
% INPUT: 
%
% img:          3D grayscale image (NO B/W bitmap)
%
...
%               A value of 0.1 to 0.2 may work for general cases.
%
...
%               be picked up.
%
% obj_cint:     (Optional, default is 0.1, range is 0-1)
%               Threshold value to keep detected objects based on center
%               intensity values. This value ranges from 0 to 1, where 0
%               keeps objects with center intensity 0 and above (all
%               objects detected) and 1 keeps objects with center intensity
%               1 or 100% of max intensity and above (no objects detected).
%
%
% OUTPUT: 
%
...
prm_grdthres = 0.2;     % gradient threshold (range 0-1)
prm_fltrLM_R = 8;       % filter radius (minimum 3)
prm_multirad = 0.5;     % multiple radius detection threshold (range 0.1-1)
prm_obj_cint = 0.1;     % object center intensity threshold (range 0 1)

% Validation of arguments
vap_grdthres = 1;   % gradient threshold
if nargin > (1 + vap_grdthres),
    if isnumeric(varargin{vap_grdthres}) && ...
            varargin{vap_grdthres}(1) >= 0 && ...
            varargin{vap_grdthres}(1) <= 1,
        prm_grdthres = varargin{vap_grdthres}(1);
    else
        error('''grdthres'' has to be within the range of 0-1');
    end
end
...
vap_multirad = 3;   % multiple radii tolerance
...
vap_obj_cint = 4;   % object center intensity
if nargin > (1 + vap_obj_cint),
    if isnumeric(varargin{vap_obj_cint}) && ...
        varargin{vap_obj_cint}(1) >= 0 && ...
        varargin{vap_obj_cint}(1) <= 1,
    prm_obj_cint = varargin{vap_obj_cint}(1);
    else
        error('''obj_cint'' has to be within the range of 0.1-1');
    end
end
...
[d1,d2,d3]=size(img);   % store image dimensions
...
[grdx, grdy, grdz] = gradient(img);
grdmag = sqrt(grdx.^2 + grdy.^2 + grdz.^2);

prm_grdthres=prm_grdthres*max(grdmag(:));  % change grdthres ratio to value

grdmasklin = find(grdmag > prm_grdthres);
[grdmask_IdxI, grdmask_IdxJ, grdmask_IdxK] = ind2sub(size(grdmag), grdmasklin);
...
lin2accum_aK = floor( ...
	double(grdz(grdmasklin)./grdmag(grdmasklin)) * linaccum_dr + ...    % Compute radius offsets
	repmat( double(grdmask_IdxK)+0.5 , [1,length(linaccum_dr)] ) ...    % Add to base position
);
lin2accum_aJ = floor( ...
	double(grdx(grdmasklin)./grdmag(grdmasklin)) * linaccum_dr + ...    % Compute radius offsets
	repmat( double(grdmask_IdxJ)+0.5 , [1,length(linaccum_dr)] ) ...    % Add to base position
);
lin2accum_aI = floor( ...
...
mask_valid_aJaIaK = ...
    lin2accum_aK > 0 & lin2accum_aK < (size(grdmag,3) + 1) & ...
...
lin2accum = sub2ind( size(grdmag), lin2accum_aI, lin2accum_aJ, lin2accum_aK);
...
prm_LM_LoBndRa = 0.1;  % minimum ratio of LM to the max of 'accum'
...
ind2=[];
count=1;
for ind=1:length(sphcen)
    tmp=sub2ind([d1,d2,d3],round(sphcen(ind,2)),round(sphcen(ind,1)),round(sphcen(ind,3)));
    if img(tmp)<prm_obj_cint*max(img(:))
        ind2(count)=ind;
        count=count+1;
    end
end
...
for k = 1 : size(sphcen,1),
    ...
    f4SgnCv = abs( ...
        double(grdx(SCvR_I0:SCvR_I1, SCvR_J0:SCvR_J1, SCvR_K0:SCvR_K1)) .* SgnCvMat_dx + ...
        double(grdy(SCvR_I0:SCvR_I1, SCvR_J0:SCvR_J1, SCvR_K0:SCvR_K1)) .* SgnCvMat_dy + ...
        double(grdz(SCvR_I0:SCvR_I1, SCvR_J0:SCvR_J1, SCvR_K0:SCvR_K1)) .* SgnCvMat_dz ...
        ) ./ SgnCvMat_r;
