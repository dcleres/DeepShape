function [FV,faceColors,ExternalIndexes,n_corners]=FindExternalVoxels(VoxelMat,Vox_Size)
...
faceColors=zeros(6*size(VoxelMat,1)*size(VoxelMat,2)*size(VoxelMat,3),1);
        ...
        for k=1:size(VoxelMat,3)
            if VoxelMat(i,j,k)==1
                if i==1 || j==1 || k==1 || i==size(VoxelMat,1) || j== size(VoxelMat,2) || k== size(VoxelMat,3)                 
                    ...
                    [FV,faceColors,FaceIndex,VertexIndex]=FindExternalFaces(VoxelMat,ExternalIndexes,voxel_size,counter,FaceIndex,VertexIndex,Vox_Size,FV,faceColors);
                    counter=counter+1;                
                else
                    if VoxelMat(i+1,j,k)==0 || VoxelMat(i-1,j,k)==0 || VoxelMat(i,j+1,k)==0 || VoxelMat(i,j-1,k)==0 || VoxelMat(i,j,k+1)==0 || VoxelMat(i,j,k-1)==0
                        ...
                        [FV,faceColors,FaceIndex,VertexIndex]=FindExternalFaces(VoxelMat,ExternalIndexes,voxel_size,counter,FaceIndex,VertexIndex,Vox_Size,FV,faceColors);
                        counter=counter+1;
                        adj_count = VoxelMat(i+1,j,k)+ VoxelMat(i-1,j,k)+ VoxelMat(i,j+1,k)+ VoxelMat(i,j-1,k) + VoxelMat(i,j,k+1) + VoxelMat(i,j,k-1);
                         if adj_count == 3
                            n_corners = n_corners+1;
                         end
                        
                         if adj_count == 1
                            n_corners = n_corners+4;
                         end
...
faceColors = faceColors(any(faceColors,2),:);

close(h) ;

ExternalIndexes = ExternalIndexes(1:counter,:);

end

function [FV,faceColors,FaceIndex,VertexIndex]=FindExternalFaces(VoxelMat,ExternalIndexes,voxel_size,i,FaceIndex,VertexIndex,LOD,FV,faceColors)
