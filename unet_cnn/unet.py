""" File unet.py containing all necessary functions to build and train a neural
network following the U-Net architecture.
# Authors: David Cleres  & jakeret"""
...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    for layer in range(0, layers):
        ...
        size -= 4
        # Followed by a max-pool, except for the last layer
        if layer < layers-1:
    ...
    for layer in range(layers-2, -1, -1):
        ...
        # Up-convolution and relu
        h_deconv = tf.nn.relu(deconv2d(in_node, wd, pool_size) + bd)
        # Concatenate the output of the corresponding level of the down layers
    ...
    # Saving step to use tensorboard
    if summaries:
...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.x = tf.placeholder("float", shape=[None, None, None, channels]) # input
        self.y = tf.placeholder("float", shape=[None, None, None, n_class]) # prediction/groundtruth
        self.keep_prob = tf.placeholder(tf.float32) #dropout (keep probability)

        # Create the U-net convolutional neural network
